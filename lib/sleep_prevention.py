...
# Each port sets this to its own app; a wrong value silently files this
# process's rows under another application's name.
_FALLBACK_APP_ID = "weidr"
...
def _app_identifier() -> str:
    ...
    Imported lazily and cached: this module lives in ``lib/`` and must not pull
    project modules in at load time. Resolution failure is logged once rather
    than silently adopting another application's identity.
    """
    ...
    if _app_id_cache is None:
        try:
            ...
            _app_id_cache = AppInfo.APP_IDENTIFIER
