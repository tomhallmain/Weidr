...
Counterpart to utils.ui_responsiveness.NullResponsiveness. Lives under ui/ so
the packages that use the port stay free of PySide6.
"""
...
class _CallableWorker(QThread):
    ...
    The exception handling is what the surrounding code already relied on: a
    failure leaves the result unset rather than reaching the caller, because an
    exception raised here would only unwind the worker thread. Logging it is
    the one addition.
    """
...
class QtResponsiveness:
    ...
    def run_off_thread(self, func: Callable[[], Any]) -> Any:
        ...
        events -- repainting, animating a spinner -- while this call blocks.
        Anything the callable invokes must therefore be safe to call from a
        worker thread.
        """
