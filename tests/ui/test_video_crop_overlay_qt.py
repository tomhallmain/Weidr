...
from ui.app_window.video_crop_overlay_qt import (
    VideoCropOverlay, launch_video_crop, launch_video_box, launch_video_background_box,
)
...
# ---------------------------------------------------------------------------
# launch_video_background_box — reuses _launch_video_rect_selector too
# ---------------------------------------------------------------------------

class TestLaunchVideoBackgroundBoxGuards:
    def test_no_player_shows_toast_and_no_overlay(self, qtbot):
        frame = MagicMock()
        frame.vlc_media_player = None
        app = MagicMock()
        launch_video_background_box(frame, "/fake/video.mp4", app)
        app.notification_ctrl.toast.assert_called_once()
        assert _mod._active_overlay is None
...
class TestLaunchVideoBackgroundBoxOverlay:
    def _make_frame(self, vid_w=1920, vid_h=1080, mf_w=960, mf_h=540):
        frame = MagicMock()
        frame.vlc_media_player.video_get_size.return_value = (vid_w, vid_h)
        frame.width.return_value = mf_w
...
class TestVideoBackgroundBoxAppliesDrawBackgroundBoxOnVideo:
    def test_enter_confirms_and_calls_draw_background_box_on_video(self, qtbot, monkeypatch):
        """Confirming a selection under launch_video_background_box must invoke
        VideoOps.draw_background_box_on_video (not crop_video / draw_box_on_video)
        with the mapped video-pixel rect."""
        import image.video_ops as _video_ops_mod

        recorded = {}

        def _fake_draw_background_box(video_path, x, y, w, h, output_path=None):
            recorded["args"] = (video_path, x, y, w, h)
            return ""  # no output produced; avoids touching MediaDetails/threading paths

        monkeypatch.setattr(
            _video_ops_mod.VideoOps, "draw_background_box_on_video", _fake_draw_background_box
        )
        monkeypatch.setattr(
            "utils.running_tasks_registry.start_thread",
            lambda fn, use_asyncio=False: fn(),
        )
        # See TestVideoBoxAppliesDrawBoxOnVideo above: start_thread runs _run()
        # synchronously, reaching QTimer.singleShot(0, app, ...) with a MagicMock
        # app. Stub QTimer so no mock ever reaches PySide6/shiboken as a receiver.
        monkeypatch.setattr(
            _mod, "QTimer", SimpleNamespace(singleShot=lambda *args: args[-1]())
        )
